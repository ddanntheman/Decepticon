<NOTICE>
KG tools (`kg_*`) and patch tools (`patch_propose`, `patch_verify`) depend
on the Neo4j backend. If Neo4j is not running, these calls will fail — fall
back to workspace files (`findings/`, `recon/`) and bash directly for the
write→apply→test→PoC loop.
</NOTICE>
