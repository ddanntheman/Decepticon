...
class TestMethodModels:
    def test_anthropic_api_full_tier_coverage(self):
        ...
        assert m[Tier.HIGH] == "anthropic/claude-fable-5"
        assert m[Tier.MID] == "anthropic/claude-opus-4-8"
        assert m[Tier.LOW] == "anthropic/claude-sonnet-5"

    def test_anthropic_oauth_routes_to_auth_prefix(self):
        m = METHOD_MODELS[AuthMethod.ANTHROPIC_OAUTH]
        assert m[Tier.HIGH] == "auth/claude-fable-5"
        assert m[Tier.MID] == "auth/claude-opus-4-8"
        assert m[Tier.LOW] == "auth/claude-sonnet-5"
...
class TestResolveChain:
    ...
    def test_oauth_then_api_high(self):
        ...
        chain = resolve_chain(Tier.HIGH, creds)
        assert chain == ["auth/claude-fable-5", "anthropic/claude-fable-5"]
    ...
    def test_oauth_then_openai_low(self):
        ...
        assert chain == ["auth/claude-sonnet-5", "openai/gpt-5-nano"]
...
class TestLLMModelMapping:
    ...
    def test_from_credentials_anthropic_only_eco(self):
        ...
        assert a.primary == "anthropic/claude-fable-5"
        assert a.fallbacks == []
    ...
    def test_from_credentials_oauth_plus_api(self):
        ...
        m = LLMModelMapping.from_credentials_and_profile(creds, ModelProfile.ECO)
        a = m.get_assignment("decepticon")
        assert a.primary == "auth/claude-fable-5"
        assert a.fallbacks == ["anthropic/claude-fable-5"]
    ...
    def test_from_credentials_full_chain_low_tier_skips_minimax(self):
        ...
        assert recon.primary == "anthropic/claude-sonnet-5"
    ...
    def test_max_profile_promotes_recon_to_high(self):
        ...
        assert m.get_assignment("recon").primary == "anthropic/claude-fable-5"
    ...
    def test_test_profile_demotes_decepticon_to_low(self):
        ...
        assert m.get_assignment("decepticon").primary == "anthropic/claude-sonnet-5"
